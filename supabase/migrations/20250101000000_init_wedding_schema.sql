/*
  # Wedding Website Database Schema - Initial Setup

  ## Overview
  Complete database schema for Emmanuel & Martha's wedding website including guest management,
  contributions tracking, pledges, meetings, attendance, and admin functionality.

  ## New Tables Created

  ### 1. `users` - Admin and staff users
  - Links to Supabase auth.users for authentication
  - Stores role-based access control (admin, finance, events, viewer)
  - Tracks user contact information

  ### 2. `guests` - Wedding guests and contributors
  - Core guest profile information
  - Phone-based identification (primary lookup)
  - Optional email and messages

  ### 3. `meetings` - Wedding preparation meetings
  - Meeting details (title, description, location, timing)
  - Created by admin users
  - Used for countdown and registration

  ### 4. `attendances` - Meeting registrations
  - Links guests to meetings
  - Tracks registration and attendance status
  - Supports walk-in registrations with standalone contact info

  ### 5. `contributions` - Monetary contributions via Pesapal
  - Real money tracking with Pesapal integration
  - Multi-currency support (default UGX)
  - Payment status tracking (pending, completed, failed)

  ### 6. `pledges` - Guest promises (money or items)
  - Flexible pledge system for money or physical items
  - Status tracking (pending, fulfilled, cancelled)
  - Fulfillment date tracking

  ### 7. `guest_messages` - Guest book wishes
  - Messages from guests to the couple
  - Moderation system (approved/pending)

  ### 8. `uploads` - Gallery images metadata
  - Tracks uploaded images
  - Links to Supabase storage buckets
  - Metadata for organization

  ### 9. `audit_logs` - Activity tracking
  - Admin and financial action logging
  - Compliance and security tracking

  ### 10. `site_settings` - Configurable site settings
  - Couple information
  ...
  - **Public Read Access**: Meetings, aggregated counts, gallery images
  - **Public Write Access**: Guest registrations, contributions, pledges (limited fields)
  - **Admin Full Access**: All tables for authenticated admin users
  - **Service Role Access**: Backend operations use service role key for privileged operations

  ## Indices

  Strategic indices added for:
  - Phone number lookups (guests, pledges, attendances)
  - Date range queries (contributions, meetings)
